...
Slices 0–5 complete (through FinBERT sentiment interface). **No OpenRouter key required** for the default path: ScriptedProvider + deterministic extractor + FakeSentiment. OpenRouter (default model `openai/gpt-4.1-nano`) and FinBERT weights are optional.
